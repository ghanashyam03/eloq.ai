...
import { OpenAICompatibleLLMProvider } from "../src/server/providers/llm/implementations/openai-compatible.provider";
...
export async function runOllamaBenchmark(datasetPath?: string): Promise<BenchmarkReportData> {
  const modelName = process.env.OLLAMA_MODEL || process.env.LOCAL_LLM_MODEL || "llama3.2:1b";
  const baseUrl = process.env.OPENAI_COMPATIBLE_BASE_URL || "http://localhost:11434/v1";

  console.log("LIVE PROVIDER: LOCAL OLLAMA");
  ...
  for (const tc of dataset.testCases) {
    ...
    try {
      ...
      for (let attempt = 1; attempt <= 2; attempt++) {
        try {
          pass1Result = await englishAnalysisService.analyzeEnglish({
            ...
            providerOverride: "local" as ProviderType,
      ...
      if (lastError || !pass1Result) {
        ...
        stoppedOnFailure = true;
        stopReason = `Local Ollama Provider failed on test case ${tc.id}: ${errMsg}`;
  ...
  const jsonReportPath = path.join(reportsDir, "ollama-benchmark-results.json");
  fs.writeFileSync(jsonReportPath, JSON.stringify(reportData, null, 2), "utf-8");

  const mdReportPath = path.join(reportsDir, "ollama-benchmark-report.md");
  ...
  const markdownContent = `# Local Ollama English Analysis Benchmark

**Provider**: local-ollama (${baseUrl})  
**Model**: ${modelName}  
...
${
  stoppedOnFailure
    ...
    : `1. **Local Privacy & Resource Economy**: Local execution on model \`${modelName}\` completed in ${avgLatencyMs}ms average latency with 0 external network requests or API costs.
2. **Linguistic Accuracy**: Model achieved ${(accuracy * 100).toFixed(1)}% overall accuracy across the 30-case benchmark.
3. **Hardware Mindful Recommendation**: \`llama3.2:1b\` uses ~1.3 GB storage and ~1.5 GB VRAM/RAM, delivering optimal performance on compact devices.`
}
`;
...
if (process.argv[1] && process.argv[1].includes("benchmark-ollama")) {
  runOllamaBenchmark().catch((err) => {
    console.error("Local Ollama Benchmark execution halted:", err);
    process.exit(1);
  });
}
